...
- **Colour contrast:** Forge Teal (#3D7A6B) measures 3.96:1 on Obsidian and 3.88:1 on Bone — both fail AA-normal (4.5:1) and pass AA-large (3:1) only. Display only at ≥24px (~1.5rem), never paragraph copy, never small UI text. Never reference `--vf-teal` directly for text; use the `--vf-color-eyebrow` semantic alias, which routes to `--vf-teal-bright` on Obsidian or `--vf-teal-deep` on Bone based on `[data-mode]` context. The same trap applies to Forge Teal as a button background: any text under 24px on a `--vf-teal` fill fails AA — filled CTAs use `--vf-teal-deep` (Bone text) at rest and `--vf-teal-bright` (Jet text) at hover, both ≥6:1.
